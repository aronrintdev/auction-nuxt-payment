<template>
  <b-container fluid class="container-product-auction h-100">
    ...
    <b-row class="mt-4">
      <b-col md="8">
        <div class="heading-1-bold">{{ editProductInventory.product.name }}</div>
        <div class="body-3-regular mt-1 text-capitalize">{{ $t('sell.confirm_listing.sku') }}: {{ editProductInventory.product.sku }}</div>
        <div class="body-3-regular mt-1 text-capitalize">{{ $t('common.colorway') }}: {{ form.color ? form.color : editProductInventory.color }}</div>
      </b-col>
      <b-col md="4">
    ...
    <b-row class="m-auto w-75">
      <b-col md="12">
        <div class="my-3 text-center">
          <ProductThumb :product="editProductInventory.product" width="350" class="my-4" />
        </div>
...
export default {
  ...
  computed:{
    ...
    sizes() {
      return this.editProductInventory?.product.sizes || []
    },
    pricesBySize() {
      const prices = []
      this.sizes.forEach(size => {
        const productPrice = this.editProductInventory?.product.product_sizes.find(sz => sz.size === size.size)
        if (productPrice) {
    ...
    packageConditions() {
      return this.packageConditionsData.map(pc => ({ value: pc.name, label: pc.name }))
    },
    colors() {
      return this.inventoryColors.map(color => ({ value: color.id, label: color.name }))
    },
    isSneakers() {
      return this.editProductInventory.product.category.name === 'sneakers'
    },
  ...
  mounted() {
    this.fetchPackagingConditions(this.editProductInventory.product.category.name)
    this.fetchColors()
    this.form = _.cloneDeep(this.editProductInventory)
  },
  methods: {
    ...mapActions({
    ...
    saveChanges() {
      ...
      this.showError = true
      if (!hasErrors) {
        this.$store.commit('create-listing/deleteItemFromNonInventoryAuctions', this.editProductInventory.id)
        this.$store.commit('create-listing/addItemToNonInventoryAuctions', {
          product: this.editProductInventory.product,
          ...this.form,
          packaging_condition: {
            name: this.form.packageCondition,
          },
          color: this.form.color ? this.form.color : this.editProductInventory.colorway,
    ...
    handleBoxConditionSelect(item) {
      ...
      this.form = {
        ...this.form,
        packageCondition: item.value,
        packaging_condition_id: item.id,
      }
    },
