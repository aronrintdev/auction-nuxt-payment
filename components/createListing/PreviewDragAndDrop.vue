<template>
  <div class="h-100 max-w828 mx-auto" @drop="dropped" @dragover.prevent="(ev) => true">
        <b-row  class="preview-drag bg-white mt-3 pb-2 px-5">
              <b-col
              ...
              <div v-if="inventories.length===0"   class="d-flex align-items-center justify-content-around w-100 preview-body my-auto ">
    ...
    <div v-if="inventories.length>0" class="d-flex justify-content-end">
      <Button pill class="mt-4 px-5" @click="$emit('next')"> {{ $t('create_listing.collection.next') }}</Button>
    </div>
  </div>
