...
export default {
  ...
  methods: {
    ...
    activeStatus(value) {
      ...
      if (!this.active.includes(value.value)) {
        this.active.push(value.value) // adding to array because value doesnt exists
      } else {
        this.active.splice(this.active.indexOf(value.value), 1) // deleting
      }
