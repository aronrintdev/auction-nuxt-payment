<template>
  <b-col md="3">
    ...
    <b-row>
      <b-col md="12">
        <b-card class="custom-card payment-form">
          ...
          <b-row class="mt-2">
            <b-col md="12">
              ...
              >
                <template #label
                  >{{
                    $t('shopping_cart.card_holder_name')
                  }}&ast;</template
                >
...
export default {
  ...
  data() {
    return {
      ...
      reserveFee: 7.99, // TODO: Temporary dummy data
    }
  },
  computed: {
    ...mapGetters({
    ...
    getSubtotal: (vm) => {
      return vm.shoppingCart.reduce((sum, product) => {
        if (product.reserve_price) {
          return sum + product.reserve_price * vm.reserveFee / 100
        }
        return sum
      }, 0)
    },
