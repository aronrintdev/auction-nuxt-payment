  ...
  <b-row>
    <b-col cols="12" sm="12">
      ...
      <b-row class="quantity-wrapper">
        <b-col v-if="getTotalQuantity > 1" cols="12" sm="12">
          <div class="body-5-normal text-gray-25 text-center text-capitalize">
            {{ getTotalQuantity }}&nbsp;{{ $tc('common.item', getTotalQuantity) }}
          </div>
        </b-col>
        <b-col v-else cols="12" sm="12">
          <div class="body-5-normal text-gray-25 text-center text-capitalize">
            {{ auction.auction_items[0].inventory.product.name }}
          </div>
        </b-col>
      </b-row>
      ...
      <b-row class="estimated-total-wrapper">
        <b-col cols="6" sm="6">
           <span class="body-5-normal text-black">{{ $t('common.total') }}&colon;</span>
        </b-col>
...
import bisSummaryMixin from '~/plugins/mixins/bid-summary'
...
export default {
  name: 'OrderSummary',
  components: { ShoppingBagTitle, ArrowLeftBlackSVG, OrderSummaryCard, PromoCodeButton, PromoCodeInput },
  mixins: [ emitEventMixin, billingAddressMixin, shippingAddressMixin, bisSummaryMixin ],
  data() {
    return {
      isLoading: false,
